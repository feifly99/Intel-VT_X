...
)
{
	...
	if (uIsMatchedTargetRip != 1)
	{
		*(ULONG_PTR*)pPte = realPhyAddAligned | 0x3ull | (oCacheType << 3);
...
)
{
	...
	if ((_rip & 0xFFFull) != (targetRipLinerAddress & 0xFFFull))
	{
...
)
{
	...
	uIsMatchedTargetRip = 0;

	targetRipLinerAddress = 0x7FF732FC104B;
	...
	ULONG_PTR targetRipPhysicalAddress = getPhysicalAddressByCR3AndVirtualAddress(targetProcessCr3, targetRipLinerAddress);
