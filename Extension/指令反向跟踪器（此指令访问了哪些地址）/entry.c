...
ULONG_PTR pRipPte, realPhyAddAligned;
...
ULONG_PTR targetRipPhysicalAddress;

ULONG_PTR targetRipLinerAddress;

ULONG* uIsTrigeredByCriticalRip;
...
VOID fixPte(ULONG_PTR pPte, ULONG_PTR physicalAddressAligned, ULONG mode)
{
	...
	case X:
		*(ULONG_PTR*)pPte = physicalAddressAligned | 0x4ull | ((ULONG64)getPhysicalMemoryCacheType(physicalAddressAligned) << 3);
		break;
	case RWX:
		*(ULONG_PTR*)pPte = physicalAddressAligned | 0x7ull | ((ULONG64)getPhysicalMemoryCacheType(physicalAddressAligned) << 3);
		break;
	default:
		KeBugCheckEx(0x44446F6F, 0, 0, 0, 0);
		break;
	}
	return;
}
...
)
{
	...
	if (uIsTrigeredByCriticalRip)
	{
		ExFreePool((PVOID)uIsTrigeredByCriticalRip);
		uIsTrigeredByCriticalRip = NULL;
	}
...
)
{
	...
	targetRipPhysicalAddress = getPhysicalAddressByCR3AndVirtualAddress(targetProcessCr3, targetRipLinerAddress);
	...
	uIsTrigeredByCriticalRip = (ULONG*)ExAllocatePoolWithTag(NonPagedPool, totalCpuCount * sizeof(ULONG), 'zzaa');
	RtlZeroMemory(uIsTrigeredByCriticalRip, totalCpuCount * sizeof(ULONG));
	...
	*(ULONG_PTR*)pPML4T = pPDPT;
	...
	for (size_t pteIndex = 0; pteIndex < PDTpages * 512 * 512; pteIndex++)
	{
		...
		{
			((ULONG_PTR*)pPT)[pteIndex] = realPhyAddAligned | 0x3ull | ((ULONG64)cacheType << 3);
			pRipPte = (ULONG_PTR)((ULONG_PTR*)pPT + pteIndex);
