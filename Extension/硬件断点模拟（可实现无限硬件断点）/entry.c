...
ULONG_PTR pPte, realPhyAddAligned;
...
)
{
	...
	*(ULONG_PTR*)pPte = realPhyAddAligned | 0x0ull | (oCacheType << 3);
	__vmx_vmwrite(VECF_PRIMARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, 0x84006172ul); //重新写入pPte.
	return;
}
...
)
{
	...
	__vmx_vmread(VEIF_EXIT_QUALIFICATION, &qualification);
	ULONG_PTR guestPhysicalAddress = 0;
	__vmx_vmread(VEIF_GUEST_PHYSICAL_ADDRESS, &guestPhysicalAddress);

	if ((targetAddress & 0xFFFull) != (guestPhysicalAddress & 0xFFFull))
	{
		*(ULONG_PTR*)pPte = realPhyAddAligned | 0x7ull | (oCacheType << 3);
		__vmx_vmwrite(VECF_PRIMARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, 0x8C006172ul); //启动MTF->目的：重新写入pPte.
		return;
	}
	else
	{
		if ((qualification & 0x1ull))
		{
			DbgPrint("access: 0x%llX\n", _rip); 
			*(ULONG_PTR*)pPte = realPhyAddAligned | 0x7ull | (oCacheType << 3);
			__vmx_vmwrite(VECF_PRIMARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, 0x8C006172ul); //启动MTF->目的：重新写入pPte.
			triggeredTimes++;
		}
		if ((qualification & 0x2ull))
		{
			DbgPrint("write: 0x%llX\n", _rip); 
			*(ULONG_PTR*)pPte = realPhyAddAligned | 0x7ull | (oCacheType << 3);
			__vmx_vmwrite(VECF_PRIMARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, 0x8C006172ul); //启动MTF->目的：重新写入pPte.
			triggeredTimes++;
		}
		if ((qualification & 0x4ull))
		{
			KeBugCheckEx(0x8888441F, 0, 0, 0, 0);
		}
	}

	if (triggeredTimes >= 50)
	{
		*(ULONG_PTR*)pPte = realPhyAddAligned | 0x7ull | (oCacheType << 3); // 取消监控（恢复READ位）.
	}
...
)
{
	...
	ULONG_PTR targetProcessCr3 = __readcr3();

	KeUnstackDetachProcess(&apc);
	ObDereferenceObject(pe);

	ULONG_PTR targetPhysicalAddress = getPhysicalAddressByCR3AndVirtualAddress(targetProcessCr3, targetAddress);
	/*DbgPrint("0x%llX\n", targetPhysicalAddress);

	LARGE_INTEGER time = { 0 };
	time.QuadPart = -10 * 1000 * 1000ll;
	...
	ULONG_PTR realPhyAdd = targetPhysicalAddress;
	...
	*(ULONG_PTR*)pPML4T = pPDPT;
	...
	for (size_t pteIndex = 0; pteIndex < PDTpages * 512 * 512; pteIndex++)
	{
		...
		((ULONG_PTR*)pPT)[pteIndex] = phyPageAddress | 0x7ull | ((ULONG64)cacheType << 3);
		if (phyPageAddress == realPhyAddAligned)
		{
			((ULONG_PTR*)pPT)[pteIndex] = realPhyAddAligned | 0x4ull | ((ULONG64)cacheType << 3);
			pPte = (ULONG_PTR)((ULONG_PTR*)pPT + pteIndex);
			oCacheType = cacheType;
		}
	}
	for (size_t i = 0; i < PDTpages * 512; i++)
	{
