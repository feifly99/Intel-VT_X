...
NTSTATUS DriverEntry(PDRIVER_OBJECT driverObject, PUNICODE_STRING regPath)
{
	...
	//ULONG totalCpuCount = KeQueryActiveProcessorCount(NULL);
	vCpu = (PVCPU)ExAllocatePoolWithTag(NonPagedPool, sizeof(VCPU), 'z+aa');
	...
	vCpu->currentCr4 = __vsm__getCR4();

	vCpu->VMX_ON_REGION_VIRTUAL_KERNEL_ADDRESS = ExAllocatePoolWithTag(NonPagedPool, regionSizeNeeded, 'vmon');
	RtlZeroMemory(vCpu->VMX_ON_REGION_VIRTUAL_KERNEL_ADDRESS, regionSizeNeeded);
	PHYSICAL_ADDRESS tempVmxon = { 0 };
	tempVmxon = MmGetPhysicalAddress(vCpu->VMX_ON_REGION_VIRTUAL_KERNEL_ADDRESS);
	vCpu->VMX_ON_REGION_PHYSICAL_ADDRESS = tempVmxon.QuadPart;

	vCpu->VMX_VMCS_REGION_VIRTUAL_KERNEL_ADDRESS = ExAllocatePoolWithTag(NonPagedPool, regionSizeNeeded, 'vmcs');
	RtlZeroMemory(vCpu->VMX_VMCS_REGION_VIRTUAL_KERNEL_ADDRESS, regionSizeNeeded);
	PHYSICAL_ADDRESS tempVmcsRegion = { 0 };
	tempVmcsRegion = MmGetPhysicalAddress(vCpu->VMX_VMCS_REGION_VIRTUAL_KERNEL_ADDRESS);
	vCpu->VMX_VMCS_REGION_PHYSICAL_ADDRESS = tempVmcsRegion.QuadPart;

	vCpu->virtualGuestStackSize = 0x11000;
	vCpu->virtualGuestStack = ExAllocatePoolWithTag(NonPagedPool, vCpu->virtualGuestStackSize, 'gstk');
	RtlZeroMemory(vCpu->virtualGuestStack, vCpu->virtualGuestStackSize);
	vCpu->virtualGuestStackBottom = (PVOID)((ULONG_PTR)vCpu->virtualGuestStack + vCpu->virtualGuestStackSize - 0x1000);

	vCpu->virtualHostStackSize = 0x11000;
	vCpu->virtualHostStack = ExAllocatePoolWithTag(NonPagedPool, vCpu->virtualHostStackSize, 'hstk');
	RtlZeroMemory(vCpu->virtualHostStack, vCpu->virtualHostStackSize);
	vCpu->virtualHostStackBottom = (PVOID)((ULONG_PTR)vCpu->virtualHostStack + vCpu->virtualHostStackSize - 0x1000);

	*(ULONG*)vCpu->VMX_ON_REGION_VIRTUAL_KERNEL_ADDRESS = vmcsIdentifier;
	*(ULONG*)vCpu->VMX_VMCS_REGION_VIRTUAL_KERNEL_ADDRESS = vmcsIdentifier;
	//开启CR4.VMXE位
	__vasm__setCR4VMXEBit();
	vCpu->currentCr4 = __vsm__getCR4();
	__vmx_on(&vCpu->VMX_ON_REGION_PHYSICAL_ADDRESS);
	//选择一个虚拟CPU的VMCS作为`current VMCS`
	KeSetSystemAffinityThread((KAFFINITY)(1 << currentVMCSCpuIndex));
	__vmx_vmclear(&vCpu->VMX_VMCS_REGION_PHYSICAL_ADDRESS);
	__vmx_vmptrld(&vCpu->VMX_VMCS_REGION_PHYSICAL_ADDRESS);
	//写入VMCS各个字段
	/*1.VM-Exit Control字段*/
	...
	__vmx_vmwrite(HOST_RSP, (PVOID)vCpu->virtualHostStackBottom);
	...
	__vmx_vmwrite(GUEST_RSP, (PVOID)vCpu->virtualGuestStackBottom);
